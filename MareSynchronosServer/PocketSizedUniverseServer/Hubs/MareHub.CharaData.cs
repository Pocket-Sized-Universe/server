...
using FileRedirectEntry = PocketSizedUniverse.API.Dto.CharaData.FileRedirectEntry;
using TorrentFileEntry = PocketSizedUniverse.API.Dto.CharaData.TorrentFileEntry;
...
public partial class MareHub
{
    ...
    public async Task<CharaDataFullDto?> CharaDataUpdate(CharaDataUpdateDto updateDto)
    {
        ...
        if (updateDto.FileRedirects != null)
        {
            var originalFiles = charaData.FileRedirects.ToList();
            charaData.FileRedirects.Clear();
            DbContext.RemoveRange(originalFiles);
            var files = charaData.FileSwaps.ToList();
            charaData.FileSwaps.Clear();
            DbContext.RemoveRange(files);
            foreach (var file in updateDto.FileRedirects)
            {
                charaData.FileRedirects.Add(new()
                {
                    GamePath = file.GamePath,
                    SwapPath = file.SwapPath,
                });
            }

            foreach (var file in updateDto.FileSwaps)
            {
                charaData.FileSwaps.Add(new()
                {
                    GamePath = file.GamePath,
                    Hash = file.Hash,
                    TorrentData = file.TorrentFile.Data,
                    FileExtension = file.TorrentFile.Extension,
                    ForbiddenBy = file.TorrentFile.ForbiddenBy,
                    IsForbidden = file.TorrentFile.IsForbidden,
                });
            }
    ...
    private static CharaDataDownloadDto GetCharaDataDownloadDto(CharaData charaData)
    {
        return new CharaDataDownloadDto(charaData.Id, charaData.Uploader.ToUserData())
        {
            ...
            FileRedirects = charaData.FileRedirects.Select(f => new FileRedirectEntry(f.SwapPath, f.GamePath)).ToList(),
            GlamourerData = charaData.GlamourerData,
            FileSwaps = charaData.FileSwaps.Select(f => new TorrentFileEntry(f.Hash, f.GamePath, new TorrentFileDto()
            {
                Data = f.TorrentData,
