...
public partial class MareWizardModule
{
    ...
    public async Task ComponentSecondary()
    {
        ...
        eb.WithDescription("You can create secondary UIDs here. " + Environment.NewLine + Environment.NewLine
            + "Secondary UIDs act as completely separate PSU accounts with their own pair list, joined syncshells, UID and so on." + Environment.NewLine
            + "Use this to create UIDs if you want to use PSU on two separate game instances at once or keep your alts private." + Environment.NewLine + Environment.NewLine
            + "__Note:__ Creating a Secondary UID is _not_ necessary to use PSU for alts." + Environment.NewLine + Environment.NewLine
    ...
    public async Task HandleAddSecondary(MareDbContext db, EmbedBuilder embed, string primaryUID)
    {
        ...
        embed.WithDescription("A secondary UID for you was created, use the information below and add the secret key to the PSU setings in the Service Settings tab.");
